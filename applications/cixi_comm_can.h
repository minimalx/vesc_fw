...
#define LAST_TWO_BITS_MASK  0x03U
...
#define CIXI_HEARTBEAT_TIMEOUT_MS 100U
...
typedef enum
{
    CIXI_ENABLE_INIT    = 0U, /**< Initial state (not used in control) */
    CIXI_ENABLE_STANDBY = 1U, /**< Standby mode: system ready but idle */
    CIXI_ENABLE_ACTIVE  = 2U, /**< Error/faulted state */
    CIXI_ENABLE_ERROR   = 3U  /**< Active state: torque command is valid */
} CixiControlEnable;
...
typedef struct
{
    float             control_value;  ///< Torque command (Nm)
    CixiControlEnable control_enable; ///< 0 = STANDBY, 3 = ACTIVE
 ...
 * data.
 */
CixiCanData cixi_get_status_data(CixiControlEnable control_enable);
