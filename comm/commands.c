...
static void (*volatile send_func_blocking)(unsigned char *data,
                                           unsigned int   len)
    = 0;
static void (*volatile send_func_nrf)(unsigned char *data, unsigned int len)
    = 0;
static void (*volatile send_func_can_fwd)(unsigned char *data, unsigned int len)
    = 0;
...
commands_init (void)
{
    ...
    chThdCreateStatic(blocking_thread_wa,
                      sizeof(blocking_thread_wa),
                      NORMALPRIO,
                      blocking_thread,
                      NULL);
    is_initialized = true;
 ...
 */
void
commands_send_packet (unsigned char *data, unsigned int len)
{
    if (send_func)
    {
        send_func(data, len);
 ...
 */
void
commands_send_packet_can_last (unsigned char *data, unsigned int len)
{
 ...
 */
void
commands_send_packet_nrf (unsigned char *data, unsigned int len)
{
    if (send_func_nrf)
    {
 ...
 */
void
commands_send_packet_last_blocking (unsigned char *data, unsigned int len)
{
...
void
commands_unregister_reply_func (void (*reply_func)(unsigned char *data,
                                                   unsigned int   len))
{
...
static void
send_func_dummy (unsigned char *data, unsigned int len)
{
    (void)data;
    (void)len;
}
 ...
 */
void
commands_process_packet (unsigned char *data,
                         unsigned int   len,
                         void (*reply_func)(unsigned char *data,
                                            unsigned int   len))
{
    ...
    switch (packet_id)
    {
        case COMM_FW_VERSION: {
            ...
            // Add 1 to the UUID for the second motor, so that configuration
        ...
        case COMM_ERASE_NEW_APP: {
            ...
            uint16_t flash_res
                = flash_helper_erase_new_app(buffer_get_uint32(data, &ind));
        ...
        case COMM_WRITE_NEW_APP_DATA_ALL_CAN:
            if (packet_id == COMM_WRITE_NEW_APP_DATA_ALL_CAN_LZO)
            {
                ...
                lzo1x_decompress_safe(send_buffer_global,
                                      len - 6,
                                      data + 4,
                                      &decompressed_len,
                                      NULL);
        ...
        case COMM_WRITE_NEW_APP_DATA: {
            ...
            uint16_t flash_res = flash_helper_write_new_app_data(
                new_app_offset, data + ind, len - ind);
        ...
        case COMM_GET_VALUES_SELECTIVE: {
            ...
            if (mask & ((uint32_t)1 << 0))
            {
                buffer_append_float16(
                    send_buffer, mc_interface_temp_fet_filtered(), 1e1, &ind);
            }
            if (mask & ((uint32_t)1 << 1))
            {
                buffer_append_float16(
                    send_buffer, mc_interface_temp_motor_filtered(), 1e1, &ind);
            }
            ...
            {
                buffer_append_float32(
                    send_buffer, mc_interface_read_reset_avg_id(), 1e2, &ind);
            }
            if (mask & ((uint32_t)1 << 5))
            {
                buffer_append_float32(
                    send_buffer, mc_interface_read_reset_avg_iq(), 1e2, &ind);
            }
            if (mask & ((uint32_t)1 << 6))
            {
                buffer_append_float16(
                    send_buffer, mc_interface_get_duty_cycle_now(), 1e3, &ind);
            }
            if (mask & ((uint32_t)1 << 7))
            {
                buffer_append_float32(
                    send_buffer, mc_interface_get_rpm(), 1e0, &ind);
            }
            ...
            {
                buffer_append_float32(
                    send_buffer, mc_interface_get_amp_hours(false), 1e4, &ind);
            }
            ...
            {
                buffer_append_float32(
                    send_buffer, mc_interface_get_watt_hours(false), 1e4, &ind);
            }
            ...
            {
                buffer_append_float32(
                    send_buffer, mc_interface_get_pid_pos_now(), 1e6, &ind);
            }
            ...
            if (mask & ((uint32_t)1 << 18))
            {
                ...
                {
                    buffer_append_float16(
                        send_buffer, NTC_TEMP_MOS1_M2(), 1e1, &ind);
                    buffer_append_float16(
                        send_buffer, NTC_TEMP_MOS2_M2(), 1e1, &ind);
                    buffer_append_float16(
                        send_buffer, NTC_TEMP_MOS3_M2(), 1e1, &ind);
                }
                else
                {
                    buffer_append_float16(
                        send_buffer, NTC_TEMP_MOS1(), 1e1, &ind);
                    buffer_append_float16(
                        send_buffer, NTC_TEMP_MOS2(), 1e1, &ind);
                    buffer_append_float16(
                        send_buffer, NTC_TEMP_MOS3(), 1e1, &ind);
                }
            }
            if (mask & ((uint32_t)1 << 19))
            {
                buffer_append_float32(
                    send_buffer, mc_interface_read_reset_avg_vd(), 1e3, &ind);
            }
            if (mask & ((uint32_t)1 << 20))
            {
                buffer_append_float32(
                    send_buffer, mc_interface_read_reset_avg_vq(), 1e3, &ind);
            }
        ...
        case COMM_SET_DUTY: {
            int32_t ind = 0;
            mc_interface_set_duty((float)buffer_get_int32(data, &ind)
                                  / 100000.0);
            timeout_reset();
        }
        break;
        ...
        case COMM_SET_CURRENT: {
            int32_t ind = 0;
            mc_interface_set_current((float)buffer_get_int32(data, &ind)
                                     / 1000.0);
            timeout_reset();
        }
        break;
        ...
        case COMM_SET_CURRENT_BRAKE: {
            int32_t ind = 0;
            mc_interface_set_brake_current((float)buffer_get_int32(data, &ind)
                                           / 1000.0);
            timeout_reset();
        }
        break;
        ...
        case COMM_SET_POS: {
            int32_t ind = 0;
            mc_interface_set_pid_pos((float)buffer_get_int32(data, &ind)
                                     / 1000000.0);
            timeout_reset();
        }
        break;
            ...
            *mcconf                  = *mc_interface_get_configuration();
            ...
            if (confgenerator_deserialize_mcconf(data, mcconf))
            {
                ...
                mcconf->lo_current_max
                    = mcconf->l_current_max * mcconf->l_current_max_scale;
                mcconf->lo_current_min
                    = mcconf->l_current_min * mcconf->l_current_min_scale;
        ...
        case COMM_GET_MCCONF_DEFAULT: {
            ...
            else
            {
                ...
                mcconf->foc_offsets_current[0]
                    = mcconf_now->foc_offsets_current[0];
                mcconf->foc_offsets_current[1]
                    = mcconf_now->foc_offsets_current[1];
                mcconf->foc_offsets_current[2]
                    = mcconf_now->foc_offsets_current[2];
                mcconf->foc_offsets_voltage[0]
                    = mcconf_now->foc_offsets_voltage[0];
                mcconf->foc_offsets_voltage[1]
                    = mcconf_now->foc_offsets_voltage[1];
                mcconf->foc_offsets_voltage[2]
                    = mcconf_now->foc_offsets_voltage[2];
        ...
        case COMM_SET_APPCONF_NO_STORE: {
            ...
            *appconf                   = *app_get_configuration();
            ...
            {
                commands_printf(
                    "Warning: Could not set appconf due to wrong signature");
            }
        ...
        case COMM_SAMPLE_PRINT: {
            ...
            mc_interface_sample_print_data(
                mode, sample_len, decimation, raw, send_func);
        }
        break;
        ...
        case COMM_GET_DECODED_PPM: {
            ...
            buffer_append_int32(
                send_buffer,
                (int32_t)(app_ppm_get_decoded_level() * 1000000.0),
                &ind);
            buffer_append_int32(
                send_buffer,
                (int32_t)(servodec_get_last_pulse_len(0) * 1000000.0),
                &ind);
            reply_func(send_buffer, ind);
        }
        break;
        ...
        case COMM_GET_DECODED_ADC: {
            ...
            buffer_append_int32(
                send_buffer,
                (int32_t)(app_adc_get_decoded_level() * 1000000.0),
                &ind);
            buffer_append_int32(send_buffer,
                                (int32_t)(app_adc_get_voltage() * 1000000.0),
                                &ind);
            buffer_append_int32(
                send_buffer,
                (int32_t)(app_adc_get_decoded_level2() * 1000000.0),
                &ind);
            buffer_append_int32(send_buffer,
                                (int32_t)(app_adc_get_voltage2() * 1000000.0),
                                &ind);
            reply_func(send_buffer, ind);
        }
        break;
        ...
        case COMM_GET_DECODED_CHUK: {
            ...
            buffer_append_int32(
                send_buffer,
                (int32_t)(app_nunchuk_get_decoded_y() * 1000000.0),
                &ind);
            reply_func(send_buffer, ind);
        }
        break;
        ...
        case COMM_GET_VALUES_SETUP_SELECTIVE: {
            ...
            {
                buffer_append_float32(
                    send_buffer, val.current_in_tot, 1e2, &ind);
            }
            if (mask & ((uint32_t)1 << 4))
            {
                buffer_append_float16(
            ...
            {
                buffer_append_float32(
                    send_buffer, mc_interface_get_speed(), 1e3, &ind);
            }
            if (mask & ((uint32_t)1 << 7))
            {
                buffer_append_float16(send_buffer,
            ...
            {
                buffer_append_float32(
                    send_buffer, val.ah_charge_tot, 1e4, &ind);
            }
            if (mask & ((uint32_t)1 << 11))
            {
            ...
            {
                buffer_append_float32(
                    send_buffer, val.wh_charge_tot, 1e4, &ind);
            }
            if (mask & ((uint32_t)1 << 13))
            {
                buffer_append_float32(
                    send_buffer, mc_interface_get_distance(), 1e3, &ind);
            }
            if (mask & ((uint32_t)1 << 14))
            {
                buffer_append_float32(
                    send_buffer, mc_interface_get_distance_abs(), 1e3, &ind);
            }
            if (mask & ((uint32_t)1 << 15))
            {
                buffer_append_float32(
            ...
            if (mask & ((uint32_t)1 << 20))
            {
                buffer_append_uint32(
                    send_buffer, mc_interface_get_odometer(), &ind);
            }
            if (mask & ((uint32_t)1 << 21))
            {
        ...
        case COMM_SET_MCCONF_TEMP_SETUP: {
            ...
            *mcconf                  = *mc_interface_get_configuration();
            ...
            mcconf->l_watt_min
                = buffer_get_float32_auto(data, &ind) / controller_num;
            mcconf->l_watt_max
                = buffer_get_float32_auto(data, &ind) / controller_num;

            // Write divided data back to the buffer, as the other controllers
            // have no way to tell how many controllers are on the bus and thus
            // need pre-divided data. We set divide by controllers to false
            // before forwarding.
            ind -= 8;
            buffer_append_float32_auto(data, mcconf->l_watt_min, &ind);
            buffer_append_float32_auto(data, mcconf->l_watt_max, &ind);

            // Battery limits can be set optionally in a backwards-compatible
            // way.
        ...
        case COMM_GET_MCCONF_TEMP: {
            ...
            *mcconf                  = *mc_interface_get_configuration();
            ...
            buffer_append_float32_auto(
                send_buffer, mcconf->l_current_min_scale, &ind);
            buffer_append_float32_auto(
                send_buffer, mcconf->l_current_max_scale, &ind);
            ...
            buffer_append_float32_auto(
                send_buffer, mcconf->l_in_current_min, &ind);
            buffer_append_float32_auto(
                send_buffer, mcconf->l_in_current_max, &ind);
            // Setup config needed for speed calculation
            send_buffer[ind++] = (uint8_t)mcconf->si_motor_poles;
            buffer_append_float32_auto(
                send_buffer, mcconf->si_gear_ratio, &ind);
            buffer_append_float32_auto(
                send_buffer, mcconf->si_wheel_diameter, &ind);
        ...
        case COMM_GET_BATTERY_CUT: {
            int32_t                          ind = 0;
            uint8_t                          send_buffer[60];
            volatile const mc_configuration *mcconf
                = mc_interface_get_configuration();

            send_buffer[ind++] = packet_id;
            buffer_append_float32(
                send_buffer, mcconf->l_battery_cut_start, 1e3, &ind);
            buffer_append_float32(
                send_buffer, mcconf->l_battery_cut_end, 1e3, &ind);
        ...
        case COMM_PSW_GET_STATUS: {
            ...
            if (stat)
            {
                ...
                buffer_append_float32_auto(
                    send_buffer, UTILS_AGE_S(stat->rx_time), &ind);
        ...
        case COMM_GET_QML_UI_HW: {
            ...
            if ((len_qml + ofs_qml) > DATA_QML_HW_SIZE
        ...
        case COMM_IO_BOARD_GET_ALL: {
            ...
            io_board_digial_inputs *digital_in
                = comm_can_get_io_board_digital_in_id(id);
            ...
            if (adc_1_4)
            {
                send_buffer[ind++] = 1;
                buffer_append_float32_auto(
                    send_buffer, UTILS_AGE_S(adc_1_4->rx_time), &ind);
                buffer_append_float16(
                    send_buffer, adc_1_4->adc_voltages[0], 1e2, &ind);
                buffer_append_float16(
                    send_buffer, adc_1_4->adc_voltages[1], 1e2, &ind);
                buffer_append_float16(
                    send_buffer, adc_1_4->adc_voltages[2], 1e2, &ind);
                buffer_append_float16(
                    send_buffer, adc_1_4->adc_voltages[3], 1e2, &ind);
            }
            ...
            if (adc_5_8)
            {
                ...
                buffer_append_float32_auto(
                    send_buffer, UTILS_AGE_S(adc_1_4->rx_time), &ind);
                buffer_append_float16(
                    send_buffer, adc_5_8->adc_voltages[0], 1e2, &ind);
                buffer_append_float16(
                    send_buffer, adc_5_8->adc_voltages[1], 1e2, &ind);
                buffer_append_float16(
                    send_buffer, adc_5_8->adc_voltages[2], 1e2, &ind);
                buffer_append_float16(
                    send_buffer, adc_5_8->adc_voltages[3], 1e2, &ind);
            }
            ...
            if (digital_in)
            {
                ...
                buffer_append_float32_auto(
                    send_buffer, UTILS_AGE_S(adc_1_4->rx_time), &ind);
                buffer_append_uint32(
                    send_buffer, (digital_in->inputs >> 32) & 0xFFFFFFFF, &ind);
                buffer_append_uint32(
                    send_buffer, (digital_in->inputs >> 0) & 0xFFFFFFFF, &ind);
            }
        ...
        case COMM_GET_STATS: {
            ...
            {
                buffer_append_float32_auto(
                    send_buffer, mc_interface_stat_speed_avg(), &ind);
            }
            if (mask & ((uint32_t)1 << 1))
            {
                buffer_append_float32_auto(
                    send_buffer, mc_interface_stat_speed_max(), &ind);
            }
            if (mask & ((uint32_t)1 << 2))
            {
                buffer_append_float32_auto(
                    send_buffer, mc_interface_stat_power_avg(), &ind);
            }
            if (mask & ((uint32_t)1 << 3))
            {
                buffer_append_float32_auto(
                    send_buffer, mc_interface_stat_power_max(), &ind);
            }
            if (mask & ((uint32_t)1 << 4))
            {
                buffer_append_float32_auto(
                    send_buffer, mc_interface_stat_current_avg(), &ind);
            }
            if (mask & ((uint32_t)1 << 5))
            {
                buffer_append_float32_auto(
                    send_buffer, mc_interface_stat_current_max(), &ind);
            }
            if (mask & ((uint32_t)1 << 6))
            {
                buffer_append_float32_auto(
                    send_buffer, mc_interface_stat_temp_mosfet_avg(), &ind);
            }
            if (mask & ((uint32_t)1 << 7))
            {
                buffer_append_float32_auto(
                    send_buffer, mc_interface_stat_temp_mosfet_max(), &ind);
            }
            if (mask & ((uint32_t)1 << 8))
            {
                buffer_append_float32_auto(
                    send_buffer, mc_interface_stat_temp_motor_avg(), &ind);
            }
            if (mask & ((uint32_t)1 << 9))
            {
                buffer_append_float32_auto(
                    send_buffer, mc_interface_stat_temp_motor_max(), &ind);
            }
            if (mask & ((uint32_t)1 << 10))
            {
                buffer_append_float32_auto(
                    send_buffer, mc_interface_stat_count_time(), &ind);
            }
        ...
        case COMM_GET_GNSS: {
            ...
            {
                buffer_append_float32_auto(
                    send_buffer, UTILS_AGE_S(g->last_update), &ind);
            }
        ...
        // own thread. If other blocking commands come before the previous one
        // has finished, they are discarded.
...
int
commands_printf_lisp (const char *format, ...)
{
    chMtxLock(&print_mutex);
...
void
commands_send_rotor_pos (float rotor_pos)
{
    uint8_t buffer[5];
    int32_t index   = 0;
...
void
commands_send_experiment_samples (float *samples, int len)
{
    if ((len * 4 + 1) > 256)
    {
        return;
...
void
commands_fwd_can_frame (int            len,
                        unsigned char *data,
                        uint32_t       id,
                        bool           is_extended)
{
...
disp_pos_mode
commands_get_disp_pos_mode (void)
{
    return display_position_mode;
}

bool
commands_set_app_data_handler (void (*func)(unsigned char *data,
                                            unsigned int   len))
{
...
void
commands_set_hw_data_handler (void (*func)(unsigned char *data,
                                           unsigned int   len))
{
    hwdata_func = func;
}
...
void
commands_send_app_data (unsigned char *data, unsigned int len)
{
    int32_t  index              = 0;
    uint8_t *send_buffer_global = mempools_get_packet_buffer();
...
void
commands_send_hw_data (unsigned char *data, unsigned int len)
{
    int32_t  index              = 0;
    uint8_t *send_buffer_global = mempools_get_packet_buffer();
...
void
commands_send_gpd_buffer_notify (void)
{
    int32_t index = 0;
    uint8_t buffer[1];
...
void
commands_send_mcconf (COMM_PACKET_ID    packet_id,
                      ...
                      void (*reply_func)(unsigned char *data, unsigned int len))
{
    ...
    int32_t len
        = confgenerator_serialize_mcconf(send_buffer_global + 1, mcconf);
    if (reply_func)
    {
...
void
commands_send_appconf (COMM_PACKET_ID     packet_id,
                                          ...
                                          unsigned int   len))
{
    ...
    send_buffer_global[0]       = packet_id;
    int32_t len
        = confgenerator_serialize_appconf(send_buffer_global + 1, appconf);
    if (reply_func)
    {
        reply_func(send_buffer_global, len + 1);
    }
...
void
commands_apply_mcconf_hw_limits (mc_configuration *mcconf)
{
    utils_truncate_number(&mcconf->l_current_max_scale, 0.0, 1.0);
    utils_truncate_number(&mcconf->l_current_min_scale, 0.0, 1.0);
    ...
    else
    {
        ...
        ctrl_loop_freq = mcconf->foc_f_zv;
#else
        utils_truncate_number(&mcconf->foc_f_zv,
                              HW_LIM_FOC_CTRL_LOOP_FREQ * 2.0);
    ...
    utils_truncate_number_abs(&mcconf->foc_sl_erpm_start,
                              mcconf->foc_sl_erpm * 0.9);

#ifndef DISABLE_HW_LIMITS

    // TODO: Maybe truncate values that get close to numerical instabilities
    // when set close to each other, such as
    //
    // conf->l_temp_motor_start, conf->l_temp_motor_end
    // and
    // conf->l_temp_fet_start, conf->l_temp_fet_end
    //
    // A division by 0 is avoided in the code, but getting close can still make
    // things oscillate. At the moment we leave the responsibility of setting
    // sane values to the user.
    ...
    utils_truncate_number(&mcconf->foc_current_filter_const,
                          HW_FOC_CURRENT_FILTER_LIM);
#endif
#endif
}

void
commands_init_plot (char *namex, char *namey)
{
    int      ind                = 0;
    uint8_t *send_buffer_global = mempools_get_packet_buffer();
...
void
commands_plot_add_graph (char *name)
{
    int      ind                = 0;
    uint8_t *send_buffer_global = mempools_get_packet_buffer();
...
void
commands_plot_set_graph (int graph)
{
    int     ind = 0;
    uint8_t buffer[2];
...
void
commands_send_plot_points (float x, float y)
{
    int32_t ind = 0;
    uint8_t buffer[10];
...
int
commands_get_fw_version_sent_cnt (void)
{
    ...
    for (;;)
    {
        ...
        switch (packet_id)
        {
            case COMM_DETECT_MOTOR_PARAM: {
                ...
                if (!conf_general_detect_motor_param(detect_current,
                                                     detect_min_rpm,
                                                     detect_low_duty,
                ...
                buffer_append_int32(send_buffer,
                                    (int32_t)(detect_cycle_int_limit * 1000.0),
                                    &ind);
                buffer_append_int32(
                    send_buffer, (int32_t)(detect_coupling_k * 1000.0), &ind);
            ...
            case COMM_DETECT_MOTOR_R_L: {
                ...
                *mcconf_old                  = *mcconf;
                ...
                // Lower f_zv means less dead time distortion and higher
                // possible current when measuring inductance on high-inductance
                // motors.
            ...
            case COMM_DETECT_MOTOR_FLUX_LINKAGE: {
                ...
                float linkage;
                bool  res = conf_general_measure_flux_linkage(
                    current, duty, min_rpm, resistance, &linkage);
            ...
            case COMM_DETECT_ENCODER: {
                if (encoder_is_configured())
                {
                    ...
                    *mcconf_old                  = *mcconf;
                    ...
                    float ratio    = 0.0;
                    bool  inverted = false;
                    mcpwm_foc_encoder_detect(
                        current, false, &offset, &ratio, &inverted);
                    mc_interface_set_configuration(mcconf_old);
            ...
            case COMM_DETECT_APPLY_ALL_FOC: {
                ...
                int res
                    = conf_general_detect_apply_all_foc_can(detect_can,
                                                            max_power_loss,
                                                            min_current_in,
                                                            max_current_in,
                                                            openloop_rpm,
            ...
            case COMM_BM_WRITE_FLASH: {
                if (packet_id == COMM_BM_WRITE_FLASH_LZO)
                {
                    ...
                    int32_t  ind              = 4;
                    lzo_uint decompressed_len = buffer_get_uint16(data, &ind);
                    lzo1x_decompress_safe(send_buffer,
                                          len - 6,
                                          data + 4,
                                          &decompressed_len,
                                          NULL);
            ...
            case COMM_BM_MAP_PINS_NRF5X: {
                ...
                buffer_append_int16(send_buffer, 1, &ind);
                bm_change_swd_pins(NRF5x_SWDIO_GPIO,
                                   NRF5x_SWDIO_PIN,
                                   NRF5x_SWCLK_GPIO,
                                   NRF5x_SWCLK_PIN);
#else
