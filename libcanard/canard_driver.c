 ...
 * May be able to remove these and only use the address for NEW_APP_BASE and
 * APP_BASE instead
 */
 ...
 * This table contains the parameters we want to be able to change via UAVCAN.
 * It is a copy of the appconf parameters with more information, the format of
 * each parameter is as follows: { parameter name, parameter type, current
...
/*
 * This function updates the local parameter value. It is called after reading
 * the current appconf data to update the local copy of the parameter.
 */
static void
updateParamByName (uint8_t *name, float value)
{
    param_t *p = NULL;
    ...
    if (p != NULL)
    {
        if (p->val != value)
        {
            if (debug_level > 0)
            {
                commands_printf("%s, %s p->val %0.02f, value %0.02f",
                                p->name,
                                name,
                                (double)p->val,
                                (double)value);
    ...
    else
    {
        ...
        {
            commands_printf(
                "UAVCAN updateParamByName(): Parameter name not found");
        }
    }
}
...
write_app_config (void)
{
    ...
    *mcconf                  = *mc_interface_get_configuration();
    ...
    appconf->can_status_rate_1
        = (uint32_t)getParamByName("can_status_rate_1")->val;
    appconf->can_status_rate_2
        = (uint32_t)getParamByName("can_status_rate_2")->val;
    appconf->can_status_msgs_r1
        = (uint16_t)getParamByName("can_status_msgs_r1")->val;
    appconf->can_status_msgs_r2
        = (uint16_t)getParamByName("can_status_msgs_r2")->val;
...
refresh_parameters (void)
{
    ...
    updateParamByName((uint8_t *)"can_status_rate_1",
                      appconf->can_status_rate_1);
    updateParamByName((uint8_t *)"can_status_rate_2",
                      appconf->can_status_rate_2);
    updateParamByName((uint8_t *)"can_status_msgs_r1",
                      appconf->can_status_msgs_r1);
    updateParamByName((uint8_t *)"can_status_msgs_r2",
                      appconf->can_status_msgs_r2);
 ...
 */
static param_t *
getParamByIndex (uint16_t index)
{
 ...
 * Searches the parameter table for the given name and returns the parameter
 * information if no parameter is found it returns null.
 */
static param_t *
getParamByName (char *name)
{
...
canard_driver_init (void)
{
    ...
    chThdCreateStatic(canard_thread_wa,
                      sizeof(canard_thread_wa),
                      NORMALPRIO,
                      canard_thread,
                      NULL);
    ...
    terminal_register_command_callback(
        "uavcan_debug",
        "Enable UAVCAN debug prints 0: off 1: errors 2: param getset 3: "
        "current calc 4: comms stuff)",
        "[level]",
        terminal_debug_on);
}

uavcan_cmd_info
canard_driver_last_rawcmd (int can_if)
{
...
uavcan_cmd_info
canard_driver_last_rpmcmd (int can_if)
{
    uavcan_cmd_info res = { 0 };
    res.age             = UTILS_AGE_S(0);
...
/*
 * Calculate the total system current consumption based on the reported
 * consumption by other ESCs transmitting data on the bus.
 */
static void
calculateTotalCurrent (void)
{
    ...
    // ToDo: Add a way to limit the ESC current based on the total system
    // current consumed
}
 ...
 */
static void
sendNodeStatus (CanardInstance *ins)
{
    node_mode = fw_update.node_id
 ...
 */
static void
sendEscStatus (CanardInstance *ins)
{
    ...
    status.power_rating_pct
        = (fabsf(mc_interface_get_tot_current()) / conf->l_current_max
           * conf->l_current_max_scale)
          * 100.0;
    status.rpm = mc_interface_get_rpm() / ((float)conf->si_motor_poles / 2.0);
    status.temperature = fmaxf(mc_interface_temp_fet_filtered(),
 ...
 */
static void
readUniqueID (uint8_t *out_uid)
{
    uint8_t len
        = UAVCAN_PROTOCOL_DYNAMIC_NODE_ID_ALLOCATION_UNIQUE_ID_MAX_LENGTH;
 ...
 * HW_VERSION is from HW_HEADER if HW_MAJOR/HW_MINOR are defined, else it sends
 * 0 This needs a change to all HW_HEADER files if desired. Unique ID is now
 * being read from the STM32 UUID field Node status is syncronize with the data
 * from the node status message.
 */
static void
handle_get_node_info (CanardInstance *ins, CanardRxTransfer *transfer)
{
    ...
    uint16_t total_size
        = uavcan_protocol_GetNodeInfoResponse_encode(&pkt, msg_buffer);
 ...
 */
static void
handle_esc_raw_command (CanardInstance *ins, CanardRxTransfer *transfer)
{
    (void)ins;
    ...
    if (uavcan_equipment_esc_RawCommand_decode_internal(
            transfer, transfer->payload_len, &cmd, &tmp, 0)
        >= 0)
    {
        if (cmd.cmd.len > app_get_configuration()->uavcan_esc_index)
        {
            float raw_val
                = ((float)
                       cmd.cmd.data[app_get_configuration()->uavcan_esc_index])
                  / 8192.0;
            ...
            switch (conf->uavcan_raw_mode)
            {
                ...
                case UAVCAN_RAW_MODE_RPM:
                    mc_interface_set_pid_speed(raw_val
                                               * conf->uavcan_raw_rpm_max);
                    break;
 ...
 */
static void
handle_esc_rpm_command (CanardInstance *ins, CanardRxTransfer *transfer)
{
    (void)ins;
    ...
    if (uavcan_equipment_esc_RPMCommand_decode_internal(
            transfer, transfer->payload_len, &cmd, &tmp, 0)
        >= 0)
    {
        ...
        {
            float rpm_val
                = cmd.rpm.data[app_get_configuration()->uavcan_esc_index];
 ...
 * Reads status messages from other ESCs on the system and stores the
 * information in memory this data can be used to respond to total system
 * current consumption.
 */
static void
handle_esc_status (CanardInstance *ins, CanardRxTransfer *transfer)
{
    (void)ins;
    ...
    if (uavcan_equipment_esc_Status_decode_internal(
            transfer, transfer->payload_len, &msg, 0, 0)
        >= 0)
    {
        for (int i = 0; i < STATUS_MSGS_TO_STORE; i++)
        {
 ...
 * VESC currently does not have a way to write parameters individually or a way
 * to access the parameters by name. So I needed to create a bridge between the
 * parameters and the UAVCAN driver which was not very clean and creates too
 * much overhead if we want to access a large number of parameters. For that
 * reason only a few paremeters were added here to allow changing of the most
 * basic features. For now any tunning that needs to be done could be done using
 * the USB interface parameters can be changed and if it needs to be deployed on
 * the field to other ESCs that are using a UAVCAN interface then a special
 * firmware with the defaults modified can be create and deployed by a UAVCAN
 * firmware update
 */
static void
handle_param_getset (CanardInstance *ins, CanardRxTransfer *transfer)
{
    ...
    if (uavcan_protocol_param_GetSetRequest_decode(
            transfer, transfer->payload_len, &req, &arraybuf_ptr)
        < 0)
    {
        return;
    }
        ...
        && req.value.union_tag != UAVCAN_PROTOCOL_PARAM_VALUE_EMPTY)
    {
        ...
        // Prevent overwrite of local parameter copy before its written to
        // eeprom
    ...
    if (p != NULL)
    {
        switch (p->type)
        {
            case AP_PARAM_INT8:
                ...
                pkt.default_value.union_tag
                    = UAVCAN_PROTOCOL_PARAM_VALUE_INTEGER_VALUE;
                pkt.min_value.union_tag
                    = UAVCAN_PROTOCOL_PARAM_VALUE_INTEGER_VALUE;
                pkt.max_value.union_tag
                    = UAVCAN_PROTOCOL_PARAM_VALUE_INTEGER_VALUE;
    ...
    uint16_t total_size
        = uavcan_protocol_param_GetSetResponse_encode(&pkt, msg_buffer);
...
/*
 * Create a Watchdog reset in order to restart the node if a restart command is
 * recieved
 */
static void
handle_restart_node (void)
{
 ...
 * This request is sent after we recieve a begin firmware udpate request, and
 * then everytime we finish processing one chunk of the firmware file. It uses
 * the fw_update.ofs value to keep track of what chunk of data needs to be
 * requested. We use fw_update.last_ms to give the server enough time to respond
 * to the previous request before we send a new one. The value is cleared after
 * procesing a response so that we dont have to wait 250ms before sending the
 * next request.
 */
static void
send_fw_read (CanardInstance *ins)
{
    fw_update.ins = ins;
...
/*
 * Handle response to file read request. This is called when we recieve a
 * response to send_fw_read() above. the packet contains a 16 bit value at the
 * begining called error that needs to be removed before reading the file chunk
 */
static void
handle_file_read_response (CanardInstance *ins, CanardRxTransfer *transfer)
{
    (void)ins;
    ...
    // Write to flash, skip the first 6 bytes for Size and CRC so need to add 6
    // always
    uint16_t flash_res
        = flash_helper_write_new_app_data(fw_update.ofs + 6, buf, len);
    fw_update.ofs += len;

    // TODO: Check result and abort on failure.
    (void)flash_res;

    // If the packet is incomplete that means that was the last packet (might be
    // an issue if the last packet is exactly full) however Ardupilot seems to
    // be handling this same way, and no issues have been reported so far.
    if (len < UAVCAN_PROTOCOL_FILE_READ_RESPONSE_DATA_MAX_LENGTH)
    {
        ...
        if (debug_level == 8)
        {
            commands_printf("UAVCAN read_response transfer finished %d kB",
                            fw_update.ofs / 1024U);
            commands_printf("new app address: 0x%lx", flash_addr[NEW_APP_BASE]);

            // Print reserved space contents for size and crc
            sizefromflash
                = buffer_get_uint32((uint8_t *)flash_addr[NEW_APP_BASE], &ind);
            crc_app
                = buffer_get_uint16((uint8_t *)flash_addr[NEW_APP_BASE], &ind);
            nextData
                = buffer_get_uint32((uint8_t *)flash_addr[NEW_APP_BASE], &ind);
        ...
        flash_res
            = flash_helper_write_new_app_data(0, sizecrc, sizeof(sizecrc));
        ...
        if (debug_level == 8)
        {
            ...
            uint16_t app_crc1
                = crc16((uint8_t *)flash_addr[APP_BASE], app_size);
            commands_printf("app crc16: 0x%02hhX", app_crc1);

            // Print size and crc data read from flash after calculation and
            // write
            ind = 0;
            sizefromflash
                = buffer_get_uint32((uint8_t *)flash_addr[NEW_APP_BASE], &ind);
            ...
            ind = 0;
            uint32_t appstartdata
                = buffer_get_uint32((uint8_t *)flash_addr[APP_BASE], &ind);
        ...
        // Do not jump directly to the bootloader after finising the transfer in
        // case we need time to allow other things to finish. Currently it only
        // delays if it needs to print the debug data.
 ...
 * for the firmware file. From this point on the client basically becomes the
 * host for the file transfer until the file is received by sending requests for
 * the next chunk of data every so often.
 */
static void
handle_begin_firmware_update (CanardInstance *ins, CanardRxTransfer *transfer)
{
    // manual decoding due to TAO bug in libcanard generated code
    if (transfer->payload_len < 1
        || transfer->payload_len > sizeof(fw_update.path) + 1)
    {
        return;
    }
    ...
    if (fw_update.node_id == 0)
    {
        ...
        {
            canardDecodeScalar(
                transfer, offset, 8, false, (void *)&fw_update.path[i]);
            offset += 8;
        }
    ...
    uint32_t total_size
        = uavcan_protocol_file_BeginFirmwareUpdateResponse_encode(&reply,
                                                                  msg_buffer);
    canardRequestOrRespond(ins,
                           transfer->source_node_id,
    ...
    if (debug_level > 0)
    {
        commands_printf("UAVCAN Begin firmware update from node_id: %d",
                        fw_update.node_id);
    }
...
/**
 * This callback is invoked by the library when a new message or request or
...
/**
 * This callback is invoked by the library when it detects beginning of a new
 * transfer on the bus that can be received by the local node. If the callback
 * returns true, the library will receive the transfer. If the callback returns
 * false, the library will ignore the transfer. All transfers that are addressed
 * to other nodes are always ignored.
 */
static bool
shouldAcceptTransfer (const CanardInstance *ins,
                      uint64_t             *out_data_type_signature,
                      uint16_t              data_type_id,
                      CanardTransferType    transfer_type,
                      uint8_t               source_node_id)
{
    ...
    //         UAVCAN_PROTOCOL_DYNAMIC_NODE_ID_ALLOCATION_SIGNATURE; return
    //         true;
    //     }
    //     return false;
    // }
    ...
    switch (data_type_id)
    {
        ...
        case UAVCAN_EQUIPMENT_ESC_RPMCOMMAND_ID:
            *out_data_type_signature
                = UAVCAN_EQUIPMENT_ESC_RPMCOMMAND_SIGNATURE;
            return true;
...
static void
terminal_debug_on (int argc, const char **argv)
{
    if (argc == 2)
    {
        int level = -1;
...
static THD_FUNCTION(canard_thread, arg)
{
    ...
    for (;;)
    {
        ...
        if (!was_running)
        {
            ...
            canardInit(&canard_ins_if2,
                       canard_memory_pool_if2,
                       sizeof(canard_memory_pool_if2),
                       onTransferReceived,
                       shouldAcceptTransfer,
                       NULL);
#endif
        ...
        while ((rxmsg = comm_can_get_rx_frame(1)) != 0)
        {
            ...
            canardHandleRxFrame(
                &canard_ins, &rx_frame, ST2US(chVTGetSystemTimeX()));
        }

        for (const CanardCANFrame *txf = NULL;
             (txf = canardPeekTxQueue(&canard_ins)) != NULL;)
        {
        ...
        while ((rxmsg = comm_can_get_rx_frame(2)) != 0)
        {
            ...
            canardHandleRxFrame(
                &canard_ins_if2, &rx_frame, ST2US(chVTGetSystemTimeX()));
        }

        for (const CanardCANFrame *txf = NULL;
             (txf = canardPeekTxQueue(&canard_ins_if2)) != NULL;)
        {
        ...
        {
            last_status_time = chVTGetSystemTimeX();
            canardCleanupStaleTransfers(&canard_ins,
                                        ST2US(chVTGetSystemTimeX()));
            sendNodeStatus(&canard_ins);
#ifdef HW_CAN2_DEV
            canardCleanupStaleTransfers(&canard_ins_if2,
                                        ST2US(chVTGetSystemTimeX()));
        ...
        if (conf->can_status_rate_1 > 0
            && UTILS_AGE_S(last_esc_status_time)
                   >= (1.0 / (float)conf->can_status_rate_1))
        {
            last_esc_status_time = chVTGetSystemTimeX();
        ...
        if (conf->can_status_rate_2 > 0
            && UTILS_AGE_S(last_esc_status_time_r2)
            ...
            >= 1000 / CURRENT_CALC_FREQ_HZ)
        {
            ...
            if (debug_level == 3)
            {
                ...
                commands_printf("Max Current: %0.02f",
                                (double)mcconf->lo_current_max);
            }
        }

        if (ST2MS(chVTTimeElapsedSinceX(last_param_refresh))
            >= 1000 / PARAM_REFRESH_RATE_HZ)
        {
            last_param_refresh = chVTGetSystemTimeX();
            ...
            {
                commands_printf(
                    "Refreshing Parameters Time Delta: %d",
                    ST2MS(chVTGetSystemTimeX() - last_param_refresh));
            }
        }

        if ((ST2MS(chVTTimeElapsedSinceX(last_read_file_req)) >= 10)
            && (fw_update.node_id != 0))
        {
            last_read_file_req = chVTGetSystemTimeX();
        ...
        if ((ST2MS(chVTTimeElapsedSinceX(jump_delay_start)) >= 500)
            && (jump_to_bootloader == true))
        {
