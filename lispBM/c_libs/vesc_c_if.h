...
typedef struct
{
    ...
    float js_y;  // Joystick Y, range -1.0 to 1.0 (this is the throttle value on
                 // most remotes)
    bool bt_c;   // Button C pressed (left on wand)
    bool bt_z;   // Button Z pressed (right on wand)
    bool is_rev; // True if the remote is in the reverse state (can be toggled
                 // on e.g the wand)
    float age_s; // Age of last update in seconds
} remote_state;

/*
 * Function pointer struct. Always add new function pointers to the end in order
 * to not break compatibility with old binaries. If a function is not available
 * (e.g. in an old firmware) it will be a null-pointer. If you make a package
 * that is meant to run on older firmware too you can check if the newer
 * functions are null pointers to know if they are available.
 */
typedef struct
{
    // LBM
    ...
    void (*free)(void *prt);
    lib_thread (*spawn)(void (*fun)(void *arg),
                        size_t stack_size,
                        char  *name,
                        void  *arg);
    ...
    // CAN
    void (*can_set_sid_cb)(bool (*p_func)(uint32_t id,
                                          uint8_t *data,
                                          uint8_t  len));
    void (*can_set_eid_cb)(bool (*p_func)(uint32_t id,
                                          uint8_t *data,
                                          uint8_t  len));
    ...
    void (*can_set_current_rel_off_delay)(uint8_t controller_id,
                                          float   current_rel,
                                          float   off_delay);
    ...
    bool (*set_app_data_handler)(void (*func)(unsigned char *data,
                                              unsigned int   len));
    ...
    void (*packet_send_packet)(unsigned char *,
                               unsigned int len,
                               PACKET_STATE_t *);
    ...
    // Terminal
    void (*terminal_register_command_callback)(const char *command,
                                               const char *help,
                                               const char *arg_names,
                                               void (*cbf)(int          argc,
                                                           const char **argv));
    void (*terminal_unregister_callback)(void (*cbf)(int          argc,
                                                     const char **argv));
    ...
    // Custom config
    void (*conf_custom_add_config)(int (*get_cfg)(uint8_t *data,
                                                  bool     is_default),
    ...
    // Mutex
    lib_mutex (*mutex_create)(
        void); // Use VESC_IF->free on the mutex when done with it
    ...
    void (*commands_unregister_reply_func)(
        void (*reply_func)(unsigned char *data, unsigned int len));

    // Set custom encoder callbacks
    void (*encoder_set_custom_callbacks)(float (*read_deg)(void),
                                         bool (*has_fault)(void),
    ...
    float (*get_ppm)(void);     // Get decoded PPM, range -1.0 to 1.0. If the
                                // decoder is not running it will be started.
    float (*get_ppm_age)(void); // Get time since a pulse was decoded in seconds
    bool (*app_is_output_disabled)(
        void); // True if apps should disable their output.
    ...
    bool (*foc_play_audio_samples)(const int8_t *samples,
                                   int           num_samp,
                                   float         f_samp,
                                   float         voltage);

    // Semaphore
    lib_semaphore (*sem_create)(
        void); // Use VESC_IF->free on the semaphore when done with it
...
#define HEADER \
    static volatile int __attribute__((__section__(".program_ptr"))) prog_ptr;
...
// The argument that was set in the init function (same as the one you get in
// stop_fun)
